...
export default function useAccountDetailsBehavior() {
  useEffect(() => {
    ...
    function renderProfile(profile) {
      ...
      const passcodeNode = document.getElementById("profile-passcode");

      if (displayName) displayName.textContent = name || "Profile";
      if (firstName) firstName.textContent = profile.firstName;
      if (lastName) lastName.textContent = profile.lastName;
      if (mobile) mobile.textContent = profile.mobile;
      if (email) email.textContent = profile.email;
      if (addressNode) addressNode.textContent = profile.address || "";
      if (passcodeNode) passcodeNode.textContent = maskPasscode(profile.passcode);
    ...
    function renderStandardEditor(key, currentValue) {
      if (key === "mobile") {
    ...
    function enterEditMode(field, key, profile) {
      ...
      if (key === "passcode") {
        editArea.innerHTML = renderPasscodeEditor();
      } else {
        editArea.innerHTML = renderStandardEditor(key, profile[key] || "");
      }
