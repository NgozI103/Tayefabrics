...
export default function useCartWishlistBehavior() {
  useEffect(() => {
    ...
    const itemIdentity = (item) =>
      `${String(item.id)}::${encodeURIComponent(
    ...
    const handleCheckoutClick = () => {
      ...
      navigateWithinApp("checkout.html");
    };
    if (checkoutBtn) {
    ...
    window.addEventListener("taye-storage-update", syncFromStorage);
    ...
    window.TayeCartWishlist = {
      addToCart(product) {
        ...
        const existing = findByIdentity(cart, itemIdentity(normalizedProduct));
        if (existing) {
      ...
      addToWishlist(product) {
        ...
        const exists = wishlist.some(
          (item) => itemIdentity(item) === itemIdentity(normalizedProduct),
        );
        if (!exists) wishlist.push(normalizedProduct);
        persist();
        renderWishlist();
      },
    };
    ...
    return () => {
      ...
      window.removeEventListener("taye-storage-update", syncFromStorage);
      if (window.TayeCartWishlist) {
        delete window.TayeCartWishlist;
      }
    };
  }, []);
}
