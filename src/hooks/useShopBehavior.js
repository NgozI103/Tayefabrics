...
export default function useShopBehavior() {
  useEffect(() => {
    ...
    function renderCategoryFilters() {
      ...
      const categoryCheckboxes = categoryFilters.querySelectorAll(
    ...
    function bindProductCardNavigation() {
      ...
      productContainer.querySelectorAll(".shop-product-card").forEach((card) => {
        const goToProduct = () => {
          ...
          if (!product) return;
          navigateWithinApp(getProductPath(product));
        };
    ...
    function renderProducts(filteredProducts) {
      ...
      if (!filteredProducts.length) {
        productContainer.innerHTML = `<p class='empty-state'>${
          searchTerm ? "Item not found." : "No products match your filters."
        }</p>`;
        return;
      }
