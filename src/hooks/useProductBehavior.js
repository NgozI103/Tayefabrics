...
import { TAYE_PRODUCTS } from "../data/products";
import { navigateWithinApp } from "../utils/navigation";
import { findProductBySlug, getProductPath } from "../utils/products";

export default function useProductBehavior() {
  useEffect(() => {
    window.TAYE_PRODUCTS = TAYE_PRODUCTS;
      ...
      optionBlock.querySelectorAll(".product-option-buttons").forEach((groupNode) => {
        groupNode.querySelectorAll(".product-option-btn").forEach((button) => {
          button.addEventListener("click", () => {
            groupNode
              .querySelectorAll(".product-option-btn")
    ...
    function collectSelectedOptions(product) {
      ...
      if (isReadyToWear(product.category)) {
        const size =
          optionBlock.querySelector('[data-option-group="size"] .product-option-btn.active')?.dataset
            .optionValue || "";
        const color =
          optionBlock.querySelector('[data-option-group="color"] .product-option-btn.active')?.dataset
            .optionValue || "";
      ...
      const length =
        optionBlock.querySelector('[data-option-group="length"] .product-option-btn.active')?.dataset
          .optionValue || "";
    ...
    function renderProductDetails(product) {
      ...
      if (detailCategory) detailCategory.textContent = product.category.toUpperCase();
      ...
      if (detailMaterial) detailMaterial.textContent = product.material || "Premium Cotton Wax";
      if (detailLength) detailLength.textContent = product.length || "6 Yards";
      if (detailOrigin) detailOrigin.textContent = product.origin || "West Africa";
      ...
      if (wishlistButton) {
        wishlistButton.addEventListener("click", () => {
          ...
          showToast(
            added ? "Added to wishlist." : "Item already in wishlist.",
            added ? "success" : "error",
          );
        });
      }
    ...
    const productIdFromQuery = Number(params.get("id"));
    ...
    if (slugFromPath) {
      selectedProduct = findProductBySlug(products, slugFromPath);
      if (!selectedProduct && /^\d+$/.test(slugFromPath)) {
        selectedProduct = products.find((product) => String(product.id) === slugFromPath) || null;
      }
    }

    if (!selectedProduct && Number.isFinite(productIdFromQuery)) {
      selectedProduct =
        products.find((product) => Number(product.id) === Number(productIdFromQuery)) || null;
    }

    if (selectedProduct) {
      const canonicalPath = getProductPath(selectedProduct);
      const canonicalParams = new URLSearchParams();
      if (intent) canonicalParams.set("intent", intent);
      const canonicalTarget = `${canonicalPath}${
        canonicalParams.toString() ? `?${canonicalParams}` : ""
      }`;
      const currentTarget = `${window.location.pathname}${window.location.search}`;
      if (currentTarget !== canonicalTarget) {
        navigateWithinApp(canonicalTarget, { replace: true });
      }
    }
