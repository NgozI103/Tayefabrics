...
export default function useLoginBehavior() {
  useEffect(() => {
      ...
      loginForm.addEventListener("submit", (event) => {
        ...
        if (normalizedEmail === ADMIN_EMAIL) {
          ...
          if (rememberMeInput && rememberMeInput.checked) {
            localStorage.setItem(REMEMBER_EMAIL_KEY, ADMIN_EMAIL);
          } else {
            localStorage.removeItem(REMEMBER_EMAIL_KEY);
          }

          updateSubmitState();
          navigateWithinApp("/admin/dashboard");
          return;
        }
